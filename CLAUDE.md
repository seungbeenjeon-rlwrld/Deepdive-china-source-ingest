...
수집 채널만 추가하려면 `--stage channels` 를 쓸 것. Stage 1·2 를 보존함.
...
- 테스트를 먼저 확인할 것: `python -m unittest discover tests` (275개)
...
- 파싱은 `src/parsing.py`, 마크다운 생성은 `src/reports.py` 에 있음. 둘 다 순수
  함수이므로 네트워크 없이 테스트할 것
