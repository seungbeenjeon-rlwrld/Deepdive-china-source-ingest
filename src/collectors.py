...
class OfficialSiteCollector:
    """Crawls a company newsroom index and preserves each article's full text."""

    def __init__(self, fetcher: Fetcher, config: OfficialSiteConfig) -> None:
        self.fetcher = fetcher
        self.config = config
        self.log = get_logger()

    def discover(self) -> list[tuple[str, str]]:
        """Walk the paginated index. Returns unique (link text, url) pairs."""
        pattern = re.compile(self.config.detail_pattern)
        found: dict[str, str] = {}
        ...
        for page in range(1, max(1, self.config.max_pages) + 1):
            ...
            if page > 1:
                ...
                url = f"{url}{joiner}{self.config.page_param}={page}"
            try:
                result = self.fetcher.fetch(url)
            except (FetchError, FetchBlocked) as exc:
                self.log.warning("index page %s failed: %s", url, exc)
                break

            before = len(found)
            for label, link in result.links:
                absolute = urljoin(result.final_url, link)
                if pattern.search(urlparse(absolute).path) and absolute not in found:
                    found[absolute] = label
            if len(found) == before:
                self.log.debug("page %d added nothing new; stopping", page)
                break

        return [(label, url) for url, label in found.items()]

    def collect(self, company: str, *, start_index: int = 1) -> tuple[list[SourceRecord], list[dict]]:
        ...
        for offset, (label, url) in enumerate(discovered):
            ...
            if page.blocked:
                failures.append({"url": url, "error": page.block_reason or "blocked"})
                continue
            if not page.text or len(page.text) < 100:
                failures.append({"url": url, "error": "no extractable body text"})
                continue

            record = SourceRecord(
                source_id=f"OFFICIAL_{start_index + offset:03d}",
                title=page.title or (label or None),
                publisher=None,  # filled from the page only if the page states it
                publication_date=page.published,
                source_platform=guess_platform(page.final_url) or urlparse(page.final_url).netloc,
                source_type="Official Company Source",
                target_company=company,
                discovery_query=f"official newsroom crawl: {self.config.index_url}",
                retrieval_url=url,
                canonical_url=page.final_url,
                url_type=None,  # not asserted; see derived.url_type_heuristic
                reaccess_status="VERIFIED_REOPENABLE",
                content_access_status="VERBATIM_FULL_TEXT",
                content=page.text,
                origin="official_site_crawl",
                extra={
                    "index_link_text": label or None,
                    "html_bytes": page.html_bytes,
                    "http_status": page.status,
                    "source_priority": "2 — Company Official Source (prompt 2 §10)",
                },
            )
            record.derived = {
                **classify_url(page.final_url),
                "content_chars": len(page.text),
                "extraction": "readability-style block scoring, link-density penalised",
            }
