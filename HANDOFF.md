...
6. 특허 출원인 탐색 및 전수 조회 (구 사명 포함, 법인명 전부) — 상위 건은 **청구항 전문**
7. 중국 로컬 도메인 조회 (정부조달·工商 등기·금융 플랫폼)
8. **검색 결과 페이지 본문 읽기** — 검색은 요약만 주므로
9. 수집 결과 저장
...
`VERBATIM_FULL_TEXT` 가 붙은 검색 결과는 파이프라인이 실제로 그 페이지를 읽은
것입니다. Baidu 가 반환하는 도메인 114종 중 53종이 평범한 요청에 본문을 내주며,
읽지 못하는 것(WeChat·工商 등기·`baijiahao`·`xueqiu`)은 스니펫으로 남고 사유가
`extra.fetch_error` 에 기록됩니다.

특허는 `extra.content_is == "claims"` 이면 본문이 **청구항**이고 초록은
`extra.abstract` 에 보관됩니다. 청구항이 법적 보호 범위를 규정하는 본문입니다.
